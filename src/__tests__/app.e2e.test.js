...
describe('E2E: Happy Tomato Calendar App', () => {
  ...
  describe('Critical Flow: Complete TODO', () => {
    test('should create a TODO and mark it complete', async () => {
      renderApp();
      ...
      // Fill description
      const descInput = screen.getByPlaceholderText(/add a description/i);
      userEvent.type(descInput, 'Water morning routine');
      ...
      // Click on the TODO event to open it
      ...
      // Find and click the complete button (check circle icon)
      const completeButton = screen.getByRole('button', { name: /complete to do/i });
      userEvent.click(completeButton);

      // Confirm completion in the confirmation modal
      await waitFor(() => {
      ...
      // Verify TODO is completed (it should be converted to a completed action)
      await waitFor(() => {
        // The TODO should be replaced with a completed action
        expect(screen.queryByText(/TO DO: Water/i)).not.toBeInTheDocument();
        // There should be a watered action or the event should be removed
      });
    });

    test('should handle canceling TODO completion', async () => {
      renderApp();
      ...
      const todoDropdown = screen.getByText(/select to-do/i);
      userEvent.click(todoDropdown);
      const todoOption = await screen.findByText(/TO DO: Fertilize/i);
      userEvent.click(todoOption);

      const descInput = screen.getByPlaceholderText(/add a description/i);
      userEvent.type(descInput, 'Weekly fertilizer application');

      await saveEvent();

      // Wait for TODO to appear
      await waitFor(() => {
        expect(screen.getByText(/TO DO: Fertilize/i)).toBeInTheDocument();
      });

      // Open the TODO
      const todoEvent = screen.getByText(/TO DO: Fertilize/i);
      userEvent.click(todoEvent);
